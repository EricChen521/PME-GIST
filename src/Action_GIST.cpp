...
Action_GIST::Action_GIST() :
  ...
  neighbor_norm_(0),
  dipole_(0),
  PME_(0), // Dataset stores Nonbond energy( Charge-charge + vdw) calculated by PME
  order_norm_(0),
  dipolex_(0),
  dipoley_(0),
  dipolez_(0),
...
# endif
  ...
  DataFile* file_energy_pme = init.DFL().AddDataFile(prefix_ + "-Etot-pme-dens" + ext);
...
void Action_GIST::NonbondEnergy_pme(Frame const& frameIn, Topology const& topIn)
{
  ...
  double* E_pme_grid = &(E_pme_[0][0]); // pointer to the E_pme_, where has the voxel-wise pme energy
...
# endif 
  ...
  **/
  ...
  double solute_sum(0); // To sum up the potential energy on solute atoms
  ...
  for ( aidx =0; aidx < maxAidx; aidx++)
  {
    ...
    {
      solute_sum += E_elec_self[a1]+ E_elec_direct[a1] + E_elec_recip[a1] + E_vdw_direct[a1] + E_vdw_self[a1] + E_vdw_recip[a1] + E_vdw_lr_cor[a1];
    }

  }

  mprintf(" The total potential energy on solute atoms: %f \n", solute_sum);
